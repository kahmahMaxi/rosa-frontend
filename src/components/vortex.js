...
export const Vortex = (props) => {
  ...
  return (
    <div className={cn("position-relative h-full w-full", props.containerClassName)}>
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        ref={containerRef}
        className="position-absolute h-full w-full inset-0 z-0 bg-transparent flex align-center justify-center"
      >
        <canvas ref={canvasRef}></canvas>
      </motion.div>

      <div className={cn("position-relative z-10", props.className)}>{props.children}</div>
    </div>
  );
};
